#!/bin/bash
dnf update -y
dnf install -y httpd php git stress-ng awscli jq

git clone https://github.com/denisegpaulo/escolatech-webapp.git
cp escolatech-webapp/* /var/www/html/
rm -rf escolatech-webapp
...
.progress-cell style(--progress > 70) {
  color: rgba(120, 0, 35, 1);
  text-shadow: 1px 1px 2px #ff2855;
  border-color: #ff2855;
}
...
.progress-cell {
    ...
    border: 1px solid #28ff55;
    border-radius: 10px;
    background: linear-gradient(to right, rgba(120, 0, 35, 1) var(--progress), rgba(0, 120, 35, 0.7) var(--progress));
}
</style>
            ...
            .then(data => {
                return new Promise((resolve, reject) => {
                    ...
                    if (data.status === 'success') {
                        stressButton.textContent = "Executando Teste de Carga de CPU";
                    }

                    setTimeout(() => resolve(data), data.status === 'success' ? 360000 : 5000);
                });
            })
            .then(data => {
